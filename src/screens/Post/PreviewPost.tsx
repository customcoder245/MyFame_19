...
const handleNext = async () => {
  ...
  try {
    // Stop the sound with proper state checking
    if (sound) {
      ...
      try {
        ...
        if (status.isLoaded) {
          ...
          console.log('Sound is not loaded, skipping stop');
        }
      } catch (soundError) {
        console.log('Error checking/stopping sound:', soundError);
        // Continue anyway - don't let sound errors block the video processing
      }
    }
    ...
    // Append audio (it's already a URL from Deezer)
    formdata.append("audio", selectedMusic); // Just pass the URL directly
    
    formdata.append("userid", profileData.id);
    formdata.append("start_time", "0");
    formdata.append("end_time", route.params.duration.toString());

    console.log('FormData created. Sending to API...');
    console.log('User ID:', profileData.id);
    console.log('Duration:', route.params.duration);
    console.log('Audio URL:', selectedMusic);
    ...
    setIsProcessing(false);
    console.log('=== HANDLE NEXT ERROR ===');
    console.log('Error:', error.message);
    
    // Don't show alert for sound-related errors, just log them
    if (error.message.includes('sound is not loaded')) {
      console.log('Sound error ignored, continuing...');
      // Try to proceed anyway by calling the API directly
      try {
        const formdata = new FormData();
        formdata.append("video", {
          uri: route.params.videoUrl,
        ...
        alert('Failed to process video: ' + apiError.message);
      }
    } else {
