...
export default function TermsOfServiceFetch({ navigation }) {
 ...
 useEffect(() => {
  ...
  const toggleTimer = () => {
    setTimer((prev) => (prev === 0 ? 3 : prev === 3 ? 5 : prev === 5 ? 10 : 0));
  };
  ...
  const handleSearchTrack = async () => {
    try {
      ...
      console.log("resultssda : ",results.data)
...
const styles = StyleSheet.create({
  ...
  timerBox: {
    ...
    justifyContent: "center",
    alignItems: "center",
    backgroundColor: "rgba(0,0,0,0.6)", // Semi-transparent background to highlight timer
    zIndex: 1, // Ensure it is above the camera view
  },
  addMoreSound: {
