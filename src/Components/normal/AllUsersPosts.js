...
useEffect(() => {
  // Only fetch if we have profileData AND we haven't loaded initial data yet
  if (profileData && profileData.id && !hasLoadedInitialData) {
    const fetchInitialVideos = async () => {
      ...
      try {
        ...
        const data = await AllUsersPostsFetch(profileData.id);
        console.log("✅ Data fetched:", data.videos?.length || 0, "videos");
...
}, [profileData]); // Remove hasLoadedInitialData from dependencies to prevent re-fetching
