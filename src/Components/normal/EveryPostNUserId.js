...
import EveryUserPostFetch from "../../Fetch_API/EveryUserPostFetch";
import VideoItem2 from "../../Navigation/Top/Components-top/VideoItem2";
import { AntDesign } from "@expo/vector-icons";
  
  export default function EveryPostNUserId(props) {
    ...
    const renderRepliesItem = ({ item }) => {
      const { profile_img, comment_author, comment_content, comment_id } = item;
    ...
    const handleLikePress = async (item, post_id, index) => {
      const updatedSources = [...imageSources];
  
      if (item.like_status) item.like_status = false;
      else item.like_status = true;
  
      if (updatedSources[index] === HeartIcon)
        item.video_likes = item.video_likes - 1;
      else item.video_likes = item.video_likes + 1;
  
      updatedSources[index] =
        updatedSources[index] === HeartIcon ? heart11 : HeartIcon;
    ...
    useFocusEffect(
        useCallback(() => {
          const fetchVideos = async () => {
            setLoading(true);
            try {
              // Fetch all videos without any profile data check
              const data = await EveryUserPostFetch(); // Assuming this API doesn't take parameters
  ...
  <FlatList
    ...
    pagingEnabled
    snapToInterval={windowHeight} // Snap based on the window height
    snapToAlignment="start"       // Ensure snapping starts at the top
    decelerationRate="fast"       // Fast deceleration for smooth snapping
    keyExtractor={(item, index) => index.toString()}
    disableIntervalMomentum={true}
    initialNumToRender={1}
    windowSize={3}                // Render more items off-screen for better performance
    onEndReachedThreshold={0.5}   // Adjust end threshold
  ...
  const styles = StyleSheet.create({
    ...
    popupContainer: {
      ...
      shadowOpacity: 0.8,
      shadowRadius: 2,
      zIndex: 1, // Ensures it overlays other elements
      flexDirection: "row",
